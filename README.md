...
- `--debug` / `--no-debug` (default: off): debug mode disables both post-run patch
  apply and artifact cleanup, preserving the full run directory for inspection. When
  debug is off (default), the winning patch is applied to `--repo` on the current branch
  and committed, and per-run artifacts are pruned to the **keep-set**: `final_report.json`,
  the winning `.diff`, `geak_agent.log`, and `COMMANDMENT.md`. Hard-kill (wall-clock
  timeout) always leaves artifacts in place regardless.
- `--total-budget-s` (default: 7200): total wall-clock budget in seconds. The hard-kill watchdog
  `os._exit(124)`s at `started_at + budget` exactly — this anchor enforces the cap.
- `--max-rounds` (default: 5): maximum number of optimization rounds. Env: `GEAK_MAX_ROUNDS`.
