...
TASK_FILE="GEAK_task.txt"
GPUS_PER_TASK=2
TOTAL_GPUS=8
START_GPU=0  # First GPU index to use
SKIP_GPUS="0,1,2"  # Comma-separated GPU IDs to skip (e.g., "5" or "3,5,7")
...
while IFS=',' read -r repo_path kernel_name || [[ -n "$repo_path" ]]; do
    # Trim leading/trailing whitespace
    repo_path=$(echo "$repo_path" | xargs)
    kernel_name=$(echo "$kernel_name" | xargs)
    ...
    geak --repo "$repo_path" \
         --task "Optimize the repository ${repo_path}, the kernel is ${kernel_name}, test command is python3 scripts/task_runner.py compile && python3 scripts/task_runner.py correctness && python3 scripts/task_runner.py performance" \
