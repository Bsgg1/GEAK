#!/usr/bin/env python3
"""GEAK v3 preprocessing test runner — Triton 3-kernel sweep.

Triton counterpart of ``scripts/preprocess_v3_test_runner.py`` (HIP).
Shares the same parent/child split + in-process orchestrator
invocation, but targets 3 Triton kernels (two from aiter, one from AKA
Triton) and inverts the cross-language assertion (Triton markers
PRESENT, HIP markers ABSENT).

Why in-process (NOT via ``geak`` subprocess):

* The CLI's ``run_pipeline`` would drive the full optimization loop
  (planner -> workers -> verifier -> patches -> finalize) which is out
  of scope for *preprocess* testing and would consume the entire 4h
  worker budget on the first kernel alone.
* In-process gives us the full :class:`PreprocessResult` dataclass —
  ``path_taken``, ``tool_calls``, ``subagent_runs``, ``errors`` —
  rather than scraping CLI logs.
* Hard wall-clock cap per scenario is enforced via
  ``subprocess.run(..., timeout=...)`` against the in-process child;
  this matches the HIP runner exactly and inherits all of its hardening
  (clean LLM-client state, isolated stdout/stderr, no signal-handling).

The runner is deliberately scenario-and-cross-language inverted from
the HIP runner so it can run in parallel without touching the HIP
worker's files. Coordination uses separate GPUs (4,5 vs HIP's 0,1) and
separate output dirs (``validation_runs/triton/...``).

Per-scenario assertions (driven by ``scenario.kind`` in the YAML):

* ``path_a`` — ``result.path_taken == "A"``; ``COMMANDMENT.md`` contains
  the user's command substring.
* ``path_a_partial`` — ``result.path_taken == "A"``; COMMANDMENT.md
  contains ``PATH_A_PARTIAL_COVERAGE`` markers for uncovered modes.
* ``path_b_determinism`` — runs the same prompt N times (default 3);
  hashes the harness from each run; pass iff all N hashes match. The
  parent also piggybacks ``path_b_coverage`` and ``cross_language`` on
  run1 of this scenario.
* ``task_override`` — single run; assert harness contains the single
  user-specified shape signature.
* ``cross_language`` — piggybacked on Path-B run1; asserts Triton
  canonical markers (``@triton.jit``, ``tl.program_id``, ...) present
  in the transcript and HIP markers (``hipLaunchKernelGGL``,
  ``__global__``, ...) absent.
* ``verifier_escalation`` — kernel fixture with renamed entry function;
  expects ``result.success == False`` + verifier-subagent retries
  logged in ``subagent_runs``.
"""
...
from preprocess_v3_triton_oracle import (  # noqa: E402
    load_oracle_for_kernel,
    shape_signature,
)
...
# Triton phrases must be PRESENT in the transcript, HIP phrases must be ABSENT.
...
def child_run_scenario(scenario_payload: dict[str, Any]) -> int:
    ...
    gpu_id = int(scenario_payload.get("gpu_id", 4))
...
def ensure_amd_llm_key() -> None:
    """Promote ``ANTHROPIC_CUSTOM_HEADERS`` to ``AMD_LLM_API_KEY`` if needed.

    Some host setups (notably Cursor's Anthropic-routing setup) expose
    the AMD gateway key inside ``ANTHROPIC_CUSTOM_HEADERS`` formatted
    ``Ocp-Apim-Subscription-Key:<hex>``. When the bare ``AMD_LLM_API_KEY``
    isn't usable, extract the subscription key and export it so the
    ``AmdLlmModel`` env-var lookup finds it. Idempotent and safe to call
    multiple times. Mirrors the HIP runner's helper of the same name
    (kept separate so each runner stays self-contained).
    """
...
def _load_model_config() -> dict[str, Any]:
    """Deep-merge ``mini_kernel_strategy_list.yaml`` <- ``geak.yaml`` model dict."""
...
def _spawn_child(
    *,
    ...
    """Run one scenario as a child process; return ``{exit_code, wall_s, ...}``."""
    payload = {
        "kernel_path": kernel_path,
        "repo_root": repo_root,
        "task": task,
...
# ---------------------------------------------------------------------------
# Harness-shape parsing (used for path_b_coverage + task_override).
# ---------------------------------------------------------------------------
...
def parse_harness_shapes(harness_path: str | Path) -> list[tuple]:
    ...
    Matches the HIP runner's contract: walk the AST, find any top-level
    assignment whose target name is in ``_SHAPE_LIST_NAMES``, decode +
    flatten its RHS to a list of leaf-tuples-of-ints. Falls back to a
    regex scan for parenthesised number sequences if AST decoding yields
    nothing (Triton harnesses sometimes embed shapes in ``triton.testing.do_bench``
    invocations instead of a top-level constant).
    """
    p = Path(harness_path)
    if not p.is_file():
        return []
...
def assert_path_a(scenario_cfg: dict[str, Any], artifacts: dict[str, Any], run_meta: dict[str, Any]) -> dict[str, Any]:
    """Path-A short-circuit + COMMANDMENT rendered (no harness file expected).

    Strictness mirrors the HIP runner: ``path_taken`` and COMMANDMENT
    existence are hard checks, ``harness_path is None`` is warn-only
    (the orchestrator occasionally surfaces ``kernel_path`` instead).
    """
...
def assert_cross_language(
    ...
    """Cross-language for Triton: Triton markers present, HIP markers absent."""
    expected_present = scenario_cfg.get("expected_phrases_present", list(TRITON_CANONICAL_PHRASES))
    expected_absent = scenario_cfg.get("expected_phrases_absent", list(HIP_CANONICAL_PHRASES))
    ...
    checks: list[dict[str, Any]] = [
        {
            "name": "expected Triton-canonical markers present in transcript",
            "ok": bool(found_present),
            "detail": f"hits={found_present}, looked_for={expected_present}",
        },
        {
            "name": "HIP-canonical markers absent from transcript",
...
def assert_path_b_run_health(
    ...
    """Per-run health check for ``path_b_determinism`` (parent rolls up later)."""
    result = artifacts.get("result") or {}
    checks: list[dict[str, Any]] = [
        {
            "name": "path_taken == B",
...
def _build_task_for_scenario(scenario: dict[str, Any], kernel: dict[str, Any]) -> str:
    """Construct the Triton-flavor task string per scenario kind.

    The phrasing is deliberately Triton-canonical (``Triton kernel``,
    ``pipeline mode: planned``, ``GPUs 4 5``) so the orchestrator's
    Step-0 indicator detection lands on Triton-language tools and the
    cross-language assertion can verify the transcript stayed on the
    Triton side of the bundle.
    """
    repo_path = kernel["repo_path"]
    kernel_url = kernel["kernel_path"]
    kernel_name = kernel["name"]
    source = kernel.get("source", "aiter")
    kind = scenario["kind"]

    if kind == "path_a":
        if source == "aka_triton":
            harness = kernel.get("harness_path", f"{repo_path}/test_kernel_harness.py")
            return (
                f"Optimize the Triton kernel at {kernel_url}. "
                f"Use the test harness at {harness}. "
                "Pipeline mode: planned — use the planner so each parallel worker receives "
                "a distinct optimization strategy. Use GPUs 4 5."
            )
        test_file = kernel.get("test_file", "")
        test_filter = kernel.get("test_filter", kernel_name)
        return (
            f"Optimize the {kernel_name} Triton kernel at {kernel_url}. Run via: "
            f"pytest {test_file} -k '{test_filter}'. "
            "Pipeline mode: planned. Use GPUs 4 5."
        )
    ...
    if kind == "path_a_partial":
        if source == "aka_triton":
            ...
            return (
                f"Optimize the Triton kernel at {kernel_url}. "
                f"Test command: python3 {harness} --benchmark. "
                "Pipeline mode: planned. Use GPUs 4 5."
            )
        test_file = kernel.get("test_file", "")
        test_filter = kernel.get("test_filter_partial") or kernel.get("test_filter", kernel_name)
        return (
            f"Optimize the {kernel_name} Triton kernel at {kernel_url}. Test command: "
            f"pytest {test_file} -k '{test_filter}'. "
            "Pipeline mode: planned. Use GPUs 4 5."
        )
        ...
        for scenario in kernel.get("scenarios", []):
            ...
            if scenario["kind"] == "path_b_determinism":
                ...
                if run_results:
                    ...
                    # them for free rather than spawning two more child runs.
...
def main() -> int:
    parser = argparse.ArgumentParser(description="GEAK v3 preprocessing test runner — Triton sweep.")
